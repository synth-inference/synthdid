---
title: "synthdid simulations"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{synthdid simulations}
...
## Summary
In this vignette, we run the simulations used in [Arkhangelsky et al.](https://arxiv.org/abs/1812.09970)
and then summarize them, computing the results displayed in its Tables 2, 3, and 4 and Figure 2. 
...
### Details
This is a long-running computation, taking roughly 3 days on a recent macbook air. It is written to save data as it runs, one file per replication, so it can be continued if interrupted. To continue, just rerun the loop: only replications for which these files are missing are run. As the %dorng% foreach loop gives each replication its own RNG state, computed as a function of the initial seed 12345, what is computed does not depend on whether the process is interrupted and continued.
...
# run simulations
set.seed(12345)
estimates.list = foreach(rr = 1:sim.replications) %dorng% { 
    ...
    if(!file.exists(filename)) {
        ...
        for(ss in 1:length(simulators)) {
            ...
            for(ee in 1:length(estimators)) {
                ...
                if(names(estimators)[ee] %in% coverage.estimators && rr <= coverage.replications) {
                    estimate[ss,ee,2] = sqrt(vcov(est, method="bootstrap")) 
                    estimate[ss,ee,3] = sqrt(vcov(est, method="jackknife"))
                    estimate[ss,ee,4] = sqrt(vcov(est, method="placebo")) 
                }
            }
        }
