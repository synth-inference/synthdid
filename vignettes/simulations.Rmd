...
* did:  diff-in-diff
* mc:   matrix completion (Athey et al.)
* difp: What does this stand for? Also called de-meaned synthetic control (cite something)
...
1. The simulator, a no-args function in the slot `$run`. 
2. The parameters of the dgp, in the slot `$params`. 
...
* We do update `ar_coef` below so `$params` has the correct coefficients. They will be shown in our summary table.
