...
## Define estimators
* sdid_reg: synthetic diff-in-diff with theoretically suggested regularization parameter:
$\zeta \sim (N_{tr} T_{post})^{1/4}$
* sdid: synthetic diff-in-diff with default regularization: $\zeta \sim 1$
* sc  : synthetic control with default regularization: $\zeta \sim 1$
* did:  diff-in-diff
* mc:   the matrix completion estimator of [Athey et al.](https://arxiv.org/abs/1710.10251)
* difp: What does this stand for? Also called de-meaned synthetic control, e.g. in [Ferman and Pinto](https://arxiv.org/abs/1911.08521)
...
estimators = list(sdid_reg=sdid_reg, 
...
estimates.list = foreach(rr = 1:sim.replications) %dorng% {
    ...
    for(ss in 1:length(simulators)) {
        ...
        for(ee in 1:length(estimators)) {
            ...
            if(names(estimators)[ee] %in% c('sdid', 'sc', 'did', 'sdid_reg') && rr <= coverage.replications) {
    ...
    print(sprintf('replication %d ran for %1.0f seconds, from %s to %s', 
                  rr, end.time-start.time, start.time, end.time))
    estimate
}

save('estimates.list', file='sims.RData')
stopCluster(cl)
```
...
# concatenates replications along the (new) 4th dimension
...
error.densities = do.call(rbind, mapply(function(ss,ee) {
    ...
    density.estimate = lindsey.density.estimate(errors, K = 100, deg = 3) 
...
show.estimators = error.densities$estimator %in% c('did', 'sc', 'sdid', 'mc')
...
ggplot(error.densities[show.sims & show.estimators, ]) + 
    ...
    theme_light() + theme(legend.position=c(.94,.92), 
