...
```{r california-weights-tables, purl=FALSE}
...
To auto-generate documentation, we'll want to rerun this notebook having already run the simulations. We'll save the results we need in 'all-simulations.rds', so if it's present we skip this step.

### Details
This is a long-running computation, taking roughly 3 days on a 2021 macbook air. It is written to save data as it runs, one file per simulation, so it can be continued if interrupted. To continue, just rerun the loop: only simulation for which these files are missing are run.
...
if(!file.exists('all-simulations.rds')) 
...
display.point.table = round(point.table[, c(1:5,8:6,10,9,15:13,17,16)], digits=3) 
display.point.table
# display table comparing default and non-default regularization choices
reg.comparison.table = round(point.table[, c(7,11,9,12)], digits=3)
reg.comparison.table 

# write tables as latex
print(xtable(display.point.table[1:14,], digits=3), type='latex', file='figures/cps-table.tex')
print(xtable(display.point.table[17:19,], digits=3), type='latex', file='figures/penn-table.tex')
print(xtable(reg.comparison.table, digits=3), type='latex', file='figures/reg-comparison-table.tex')
```
...
display.coverage.table = round(coverage.table[c(1,7:19), c(3:1,10:8,17:15)], digits=2)
display.coverage.table

print(xtable(display.coverage.table, align='|l|rrr|rrr|rrr|'), 
             hline.after = c(1,6,9,11,14) , NA.string = '---',
      type='latex', file='figures/coverage-table.tex')
```
