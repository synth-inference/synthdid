...
```{r imports-notebook-only, echo=FALSE, purl=FALSE}
library(dplyr)
library(tidyr)
library(tibble)
library(ggplot2)
...
It can also be run on a slurm cluster: set `cluster=TRUE` below and ensure the template file batchtools.slurm.tmpl
is correctly configured. We include one for the Sherlock cluster at Stanford.

The loop gives each replication of a simulation its own RNG seed, computed as 
...
if(cluster) {
    ...
    plan(batchtools_slurm, resources=list(ntasks=100, ncpus=1, memory=1024))
} else {
