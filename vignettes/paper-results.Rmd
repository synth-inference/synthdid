...
estimates.list = foreach(rr = 1:sim.replications) %dopar% { 
    ...
    if(!file.exists(filename)) {
        ...
        if(rr <= coverage.replications) {
            estimate.corrected = run.replication(seeds[[rr]], coverage.for = 'difp')
            estimate[,names(estimators)=='difp',,]=estimate.corrected[,names(estimators)=='difp',,]
        }
        saveRDS(estimate, file=sprintf('replications-corrected/replication%d.RDS', rr))
