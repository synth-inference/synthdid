...
epsilon = 1e-6
synthdid.reference = function(Y, N0, T0, zeta.omega=sigma.default(Y, N0, T0)) {
    N = nrow(Y); T=ncol(Y); N1 = N-N0; T1=T-T0;
    lambda = simplex.least.squares(Y[1:N0, 1:T0],    rowMeans(Y[1:N0, (T0+1):T, drop=FALSE]), zeta=epsilon*zeta.omega, intercept=TRUE)
...
sc.reference = function(Y, N0, T0, zeta.omega=sigma.default(Y, N0, T0)) {
    N = nrow(Y); T=ncol(Y); N1 = N-N0; T1=T-T0;
